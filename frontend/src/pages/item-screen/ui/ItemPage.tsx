...
interface ItemViewProps {
    ...
    onBack: () => void;
    onDelete: () => void;
    onUpdate: (name: string, category: string, description: string, photo?: string) => void;
}
export function ItemView({ item, onBack, onDelete, onUpdate }: ItemViewProps) {
    ...
    const handleSave = () => {
        ...
        onUpdate(editName.trim(), editCategory.trim(), editDescription.trim(), editPhoto);
        setIsEditing(false);
    };
    const handleCancel = () => {
        setEditName(item.name);
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              {displayPhoto ? (<div className="relative group">
                  <img src={displayPhoto} alt={item.name} className="w-full max-h-72 object-contain rounded-lg border bg-muted"/>
                ...
                </div>) : (<button type="button" onClick={isEditing ? () => fileInputRef.current?.click() : undefined} disabled={!isEditing} className="w-full border-2 border-dashed rounded-lg p-10 flex flex-col items-center justify-center gap-2 text-muted-foreground transition-colors disabled:cursor-default enabled:hover:border-muted-foreground enabled:hover:text-foreground enabled:cursor-pointer">
            ...
            <div className="space-y-2">
              ...
              {isEditing ? (<div className="space-y-1">
                ...
                </div>) : (<p className="p-3 bg-muted rounded">{item.name}</p>)}
            </div>

            <div className="space-y-2">
              <Label htmlFor="item-category">Категория</Label>
              {isEditing ? (<Input id="item-category" value={editCategory} onChange={(e) => setEditCategory(e.target.value)} placeholder="Например: Одежда, Инструменты, Книги"/>) : (<p className="p-3 bg-muted rounded">
            ...
            <div className="space-y-2">
              ...
              {isEditing ? (<Textarea id="item-description" value={editDescription} onChange={(e) => setEditDescription(e.target.value)} placeholder="Дополнительная информация о предмете" rows={4}/>) : (<p className="p-3 bg-muted rounded min-h-[100px] whitespace-pre-wrap">
