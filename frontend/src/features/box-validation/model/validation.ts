...
/**
 * Checks that a box (in cm) fits within the storage room (in metres).
 * Converts room metres → centimetres before comparing.
 */
export function validateBoxFitsInRoom(
  box: BoxDimensions,
  room: RoomDimensions,
): BoxValidationResult {
  const errors: string[] = [];
  ...
  if (box.sizeW < MIN_BOX_DIMENSION_CM) {
    errors.push(
      `Длина коробки не может быть меньше ${MIN_BOX_DIMENSION_CM} см`,
    );
  }
  if (box.sizeD < MIN_BOX_DIMENSION_CM) {
    errors.push(
      `Ширина коробки не может быть меньше ${MIN_BOX_DIMENSION_CM} см`,
    );
  }
  if (box.sizeH < MIN_BOX_DIMENSION_CM) {
    errors.push(
      `Высота коробки не может быть меньше ${MIN_BOX_DIMENSION_CM} см`,
    );
  }

  if (box.sizeW > roomWcm) {
    errors.push(
      `Длина коробки (${box.sizeW} см) превышает длину комнаты (${roomWcm} см)`,
    );
  }
  if (box.sizeD > roomDcm) {
    errors.push(
      `Ширина коробки (${box.sizeD} см) превышает ширину комнаты (${roomDcm} см)`,
    );
  }
  if (box.sizeH > roomHcm) {
    errors.push(
      `Высота коробки (${box.sizeH} см) превышает высоту комнаты (${roomHcm} см)`,
    );
  }
...
export function findBoxesExceedingRoom<T extends BoxDimensions>(
  boxes: T[],
  room: RoomDimensions,
): T[] {
...
export function validateStorageRoomFitsAllBoxes(
  boxes: NamedBoxDimensions[],
  room: RoomDimensions,
): BoxValidationResult {
...
export function isDuplicateBoxName(
  name: string,
  ...
  return boxes.some(
    (b) => b.name.trim().toLowerCase() === normalized && b.id !== excludeBoxId,
  );
}
