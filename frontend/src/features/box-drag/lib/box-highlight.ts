...
export function getBoxHighlightClass(state: BoxHighlightState): string {
    const { isDragging, isSearchMatch, isSearchHighlight, isFocused, isStackExpanded, } = state;
    if (isDragging)
        return "outline outline-2 outline-blue-500 outline-offset-0";
    if (isSearchMatch)
    ...
    if (isFocused)
        return "outline outline-2 outline-violet-500 outline-offset-0";
    if (isStackExpanded)
