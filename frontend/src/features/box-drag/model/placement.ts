...
function footprintOverlapArea(x: number, y: number, box: BoxDims, ox: number, oy: number, ow: number, od: number): number {
    const overlapW = Math.max(0, Math.min(x + box.sizeW, ox + ow) - Math.max(x, ox));
    const overlapD = Math.max(0, Math.min(y + box.sizeD, oy + od) - Math.max(y, oy));
    return overlapW * overlapD;
}
export function overlapsPartitionFootprint(x: number, y: number, box: BoxDims, partitions: PartitionDims[]): boolean {
    return partitions.some((p) => footprintOverlapArea(x, y, box, p.x, p.y, p.width, p.depth) > 0);
}
export function isInPartitionGap(x: number, y: number, box: BoxDims, partitions: PartitionDims[]): boolean {
    for (let i = 0; i < partitions.length; i++) {
        for (let j = i + 1; j < partitions.length; j++) {
            const a = partitions[i];
            const b = partitions[j];
            const yOverlap = Math.max(0, Math.min(y + box.sizeD, Math.max(a.y + a.depth, b.y + b.depth)) -
            ...
            if (gapWidth <= 0 || gapWidth >= box.sizeW)
                continue;
            if (x + box.sizeW > gapStart && x < gapEnd)
                return true;
        }
    }
    return false;
}
export function computeRestingZ(x: number, y: number, box: BoxDims, placedBoxes: PlacedBoxDims[], partitions: PartitionDims[] = [], maxSupportZ?: number): number {
    void partitions;
    ...
    for (const b of others) {
        ...
        if (area > 0) {
            ...
            if (maxSupportZ !== undefined && top > maxSupportZ)
                continue;
...
export function overlapsZone(x: number, y: number, box: BoxDims, zone: XYZone | null | undefined): boolean {
    if (!zone)
        return false;
...
export function has3DConflict(x: number, y: number, z: number, box: BoxDims, placedBoxes: PlacedBoxDims[], partitions: PartitionDims[] = []): boolean {
...
function xyOverlapArea(a: PlacedBoxDims, b: PlacedBoxDims): number {
    const ow = Math.max(0, Math.min(a.x + a.sizeW, b.x + b.sizeW) - Math.max(a.x, b.x));
    const od = Math.max(0, Math.min(a.y + a.sizeD, b.y + b.sizeD) - Math.max(a.y, b.y));
    return ow * od;
}
...
function areVerticallyAdjacent(a: PlacedBoxDims, b: PlacedBoxDims): boolean {
    ...
    return (Math.abs(aTop - b.z) <= STACK_Z_EPSILON_CM ||
...
export function computeSettleUpdates(movedBoxOldPos: PlacedBoxDims, newPlacedBoxes: PlacedBoxDims[], partitions: PartitionDims[] = []): Array<{
    id: string;
    x: number;
    y: number;
    z: number;
    ...
    const affected = newPlacedBoxes.filter((b) => {
        const xOk = b.x < movedBoxOldPos.x + movedBoxOldPos.sizeW &&
            b.x + b.sizeW > movedBoxOldPos.x;
        const yOk = b.y < movedBoxOldPos.y + movedBoxOldPos.sizeD &&
    ...
    for (const ab of affected) {
        ...
        if (Math.abs(newZ - ab.z) > STACK_Z_EPSILON_CM) {
            ...
            if (idx >= 0)
                settledList[idx] = { ...ab, z: newZ };
        }
    }
    return updates;
}
export function findNearestValidXY(desiredX: number, desiredY: number, box: BoxDims, placedBoxes: PlacedBoxDims[], room: {
    widthCm: number;
    depthCm: number;
    heightCm: number;
}, forbiddenZone?: XYZone | null, partitions: PartitionDims[] = []): {
    x: number;
    y: number;
    z: number;
} | null {
    const tryPos = (rx: number, ry: number): {
        ...
        if (overlapsZone(cx, cy, box, forbiddenZone))
            return null;
        if (isInPartitionGap(cx, cy, box, partitions))
            return null;
        if (overlapsPartitionFootprint(cx, cy, box, partitions))
            return null;
        const z = computeRestingZ(cx, cy, box, placedBoxes, partitions);
        if (z + box.sizeH > room.heightCm)
            return null;
        if (has3DConflict(cx, cy, z, box, placedBoxes, partitions))
            return null;
        return { x: cx, y: cy, z };
    };
    ...
    if (blocking.length > 0) {
        ...
        edgeCandidates.sort((ca, cb) => Math.hypot(ca[0] - desiredX, ca[1] - desiredY) -
