...
import { computeRestingZ, computeSettleUpdates, has3DConflict, findNearestValidXY, overlapsZone, type BoxDims, type PlacedBoxDims, type XYZone, type PartitionDims, } from "./placement";
...
export interface BoxDragState {
    ...
    handleDragStart: (e: React.DragEvent, boxId: string, fromCanvas?: boolean, overrideOffsetPx?: {
        pxX: number;
        pxY: number;
    }) => void;
    handleDragEnd: () => void;
...
function isPlacedBox(b: BoxDims & {
    x?: number;
    y?: number;
    z?: number;
}): b is PlacedBoxDims {
...
export function useBoxDrag({ boxes, room, callbacks, forbiddenZoneRef, partitions = [], }: UseBoxDragOptions): BoxDragState {
    ...
    const getCanvasCmFromEvent = useCallback((e: React.DragEvent): {
        xCm: number;
        yCm: number;
    } | null => {
        if (!gridRef.current)
    ...
    const clampPos = (xCm: number, yCm: number, box: BoxDims): {
        xCm: number;
        yCm: number;
    } => ({
    ...
    const handleDragStart = useCallback((e: React.DragEvent, boxId: string, fromCanvas = false, overrideOffsetPx?: {
        pxX: number;
        pxY: number;
    }) => {
        if (overrideOffsetPx) {
    ...
    const handleGridDrop = useCallback((e: React.DragEvent) => {
        ...
        if (!raw)
            return;
        if (forbiddenZoneRef?.current &&
            overlapsZone(raw.xCm, raw.yCm, draggedBox, forbiddenZoneRef.current)) {
            return;
        }
        const result = findNearestValidXY(raw.xCm, raw.yCm, draggedBox, placedBoxes, room, null, partitions);
        if (!result)
            return;
    ...
    }, [boxes, placedBoxes, room, onMoveBox, forbiddenZoneRef, partitions, getCanvasCmFromEvent]);
    ...
    const handleStagingDrop = useCallback((e: React.DragEvent) => {
        e.preventDefault();
        const boxId = e.dataTransfer.getData("text/plain");
        setDraggedBoxId(null);
        setDragOverCm(null);
