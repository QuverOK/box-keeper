...
function PasswordChecklist({ password, visible, }: {
    ...
    return (<AnimatePresence initial={false}>
      {visible && (<motion.ul className="mt-2 space-y-1" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} style={{ overflow: "hidden" }}>
          {checks.map((check, i) => (<motion.li key={check.key} custom={i} variants={checklistItemVariants} initial="hidden" animate="visible" className={`flex items-center gap-1.5 text-xs ${check.passed ? "text-green-600" : "text-gray-400"}`}>
              {check.passed ? (<Check className="w-3.5 h-3.5 flex-shrink-0"/>) : (<X className="w-3.5 h-3.5 flex-shrink-0"/>)}
              {check.label}
            </motion.li>))}
        </motion.ul>)}
    </AnimatePresence>);
...
export function LoginRegister({ onLogin, onScanQr }: LoginRegisterProps) {
    ...
    return (<div className="min-h-screen flex items-center justify-center bg-background p-4">
      <motion.div className="w-full max-w-md" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28, ease: "easeOut" }}>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          ...
          <TabsContent value="login">
            <motion.div key={tabKeys.login} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18, ease: "easeOut" }}>
              <Card>
            ...
            setLoginError("");
                  ...
                  <CardFooter>
                    <motion.div className="w-full" whileTap={{ scale: 0.97 }} transition={{ duration: 0.1 }}>
                      <Button type="submit" className="w-full mt-3" disabled={loginLoading}>
          ...
          <TabsContent value="register">
            <motion.div key={tabKeys.register} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18, ease: "easeOut" }}>
              <Card>
                <form onSubmit={handleRegister}>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="register-name" type="text" placeholder="Иван Иванов" value={registerName} onChange={(e) => setRegisterName(e.target.value)}/>
                    </div>
                    <div className="space-y-2">
            ...
            registerPassword !== registerConfirmPassword && (<p className="text-xs text-red-500 mt-1">
                  ...
                  <CardFooter>
                    <motion.div className="w-full" whileTap={{ scale: 0.97 }} transition={{ duration: 0.1 }}>
                      <Button type="submit" className="w-full mt-3" disabled={registerLoading}>
