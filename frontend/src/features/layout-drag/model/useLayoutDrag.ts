...
export function useLayoutDrag({ room, partitions, callbacks, gridRef, }: UseLayoutDragOptions): LayoutDragState {
    ...
    const handleDragStart = useCallback((e: React.DragEvent, item: LayoutDragItem) => {
        ...
        const prefix = item.kind === "partition"
    ...
    const handleGridDrop = useCallback((e: React.DragEvent) => {
        ...
        if (payload.kind === "partition") {
            ...
            if (!partition)
                return;
            const { xCm, yCm } = clampPartitionXY(raw.xCm, raw.yCm, partition.width, partition.depth, room);
