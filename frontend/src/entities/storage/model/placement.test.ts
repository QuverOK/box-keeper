...
describe("computeRestingZ", () => {
  it("returns 0 when no boxes are below", () => {
    const z = computeRestingZ(
      { posX: 0, posY: 0, sizeW: 60, sizeD: 60, sizeH: 40 },
      [],
    );
    expect(z).toBe(0);
  });
  it("returns top of overlapping box", () => {
    ...
    const z = computeRestingZ(
      { posX: 0, posY: 0, sizeW: 60, sizeD: 60, sizeH: 30 },
      [existing],
    );
    expect(z).toBe(40);
  });
  it("stacks correctly on the tallest overlapping box", () => {
    ...
    const z = computeRestingZ(
      { posX: 0, posY: 0, sizeW: 30, sizeD: 30, sizeH: 20 },
      [box1, box2],
    );
    expect(z).toBe(90);
  });
...
describe("boxFitsInStorage", () => {
  ...
  it("returns true when box fits within bounds", () => {
    expect(
      boxFitsInStorage(
        { posX: 0, posY: 0, posZ: 0, sizeW: 60, sizeD: 80, sizeH: 40 },
        room,
      ),
    ).toBe(true);
  });
  it("returns false when box exceeds room width", () => {
    expect(
      boxFitsInStorage(
        { posX: 0, posY: 0, posZ: 0, sizeW: 700, sizeD: 80, sizeH: 40 },
        room,
      ),
    ).toBe(false);
  });
  it("returns false when box position puts it outside bounds", () => {
    expect(
      boxFitsInStorage(
        { posX: 550, posY: 0, posZ: 0, sizeW: 60, sizeD: 80, sizeH: 40 },
        room,
      ),
    ).toBe(false);
  });
