...
export function AppHeader() {
    ...
    return (<motion.header className="bg-card border-b sticky top-0 z-10" initial={{ y: -8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.2, ease: "easeOut" }}>
      <div className="page-container py-4 flex items-center justify-between">
        <motion.div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate({ to: "/dashboard" })} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 400, damping: 20 }}>
        ...
        <nav className="flex items-center gap-1">
          ...
          <motion.div variants={navBtnVariants} initial="rest" whileHover="hover" whileTap="tap" transition={{ type: "spring", stiffness: 400, damping: 17 }}>
            <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/qr" })}>
