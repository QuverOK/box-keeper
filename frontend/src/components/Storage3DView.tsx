...
import { Box as BoxIcon, Layers, GripVertical, Inbox, ExternalLink, Package, } from "lucide-react";
...
export function Storage3DView({ boxes, onBoxClick, editMode, highlightedBoxId, gridSize: _gridSize, onMoveBox, roomSize, }: Storage3DViewProps) {
    void _gridSize;
    ...
    const getBoxCanvasPos = (box: PlacedBox): {
        xCm: number;
        yCm: number;
        depth: number;
    } => {
    ...
    const getBoxVisualDims = (box: Box): {
        wCm: number;
        hCm: number;
        depthCm: number;
    } => {
    ...
    const getPlacementCoords3D = (canvasXCm: number, canvasYCm: number, existingDepth?: number): {
        x: number;
        y: number;
        z: number;
    } => {
    ...
    const computeRestingZ = (x: number, y: number, box: Box): number => {
        ...
        for (const b of others) {
            const ow = Math.max(0, Math.min(x + box.sizeW, b.x + b.sizeW) - Math.max(x, b.x));
            const od = Math.max(0, Math.min(y + box.sizeD, b.y + b.sizeD) - Math.max(y, b.y));
            const area = ow * od;
            if (area > 0) {
    ...
    const has3DConflict = (x: number, y: number, z: number, box: Box): boolean => placedBoxes.some((b) => {
        if (b.id === box.id)
            return false;
    ...
    const findNearestValidXY = (desiredX: number, desiredY: number, box: Box): {
        x: number;
        y: number;
        z: number;
    } | null => {
        const roomHeightCm = roomSize.height * 100;
        const tryPos = (rx: number, ry: number): {
            x: number;
            y: number;
            z: number;
        } | null => {
            ...
            if (z + box.sizeH > roomHeightCm)
                return null;
            if (has3DConflict(cx, cy, z, box))
                return null;
            return { x: cx, y: cy, z };
        };
        ...
        for (let radius = 5; radius <= 200; radius += 5) {
            ...
            for (let i = 0; i < numAngles; i++) {
                ...
                const result = tryPos(desiredX + radius * Math.cos(angle), desiredY + radius * Math.sin(angle));
                if (result)
                    return result;
            }
        }
    ...
    const getCanvasCmFromEvent = (e: React.DragEvent): {
        xCm: number;
        yCm: number;
    } | null => {
        if (!gridRef.current)
    ...
    const clampPos = (xCm: number, yCm: number, box: Box): {
        xCm: number;
        yCm: number;
    } => {
    ...
    const handleDragStart = (e: React.DragEvent, boxId: string, fromCanvas = false) => {
        if (fromCanvas) {
    ...
    const draggedBoxPlaced = draggedBoxDisplay && isPlaced(draggedBoxDisplay) ? draggedBoxDisplay : null;
    const ghost = (() => {
        if (!editMode || !dragOverCm || !draggedBoxDisplay)
            return null;
        const { wCm, hCm } = getBoxVisualDims(draggedBoxDisplay);
        const { xCm, yCm } = clampPos(dragOverCm.xCm, dragOverCm.yCm, draggedBoxDisplay);
    ...
    return (<div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-4 items-center justify-between bg-white p-4 rounded-lg border">
        <div className="flex gap-2">
          <Button variant={viewMode === "XY" ? "default" : "outline"} onClick={() => setViewMode("XY")} className="gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-4">
        <div className="space-y-4">
          ...
          <Card className={[
            ...
            editMode && isStagingDragOver
                ...
                : "",
        ].join(" ")} onDragOver={editMode ? handleStagingDragOver : undefined} onDragLeave={editMode ? handleStagingDragLeave : undefined} onDrop={editMode ? handleStagingDrop : undefined}>
            <div className="flex items-center gap-2 mb-3">
              ...
              {editMode && isStagingDragOver && (<span className="ml-auto text-xs text-blue-600 font-medium">
                  Отпустите, чтобы убрать с холста
                </span>)}
            </div>

            {unplacedBoxes.length === 0 ? (<div className={[
                ...
                return (<div key={box.id} className="flex-shrink-0" draggable={editMode} onDragStart={editMode
                      ...
                      <Card className={[
                        ...
                        highlightedBoxId === box.id
                            ...
                            : "",
                    ].join(" ")} style={{ backgroundColor: box.color }} onClick={() => setFocusedBoxId(box.id)}>
                        {editMode && (<div className="absolute top-1 left-1 text-gray-500 opacity-50">
                ...
                {editMode && (<div className="flex-shrink-0 w-28 h-24 border-2 border-dashed border-gray-200 rounded-lg flex items-center justify-center">
          ...
          <Card className="p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              {editMode && (<span className="text-sm text-blue-600 font-medium bg-blue-50 px-3 py-1 rounded-full border border-blue-200">
            ...
            paddingBottom: `clamp(280px, ${aspectRatioPct.toFixed(2)}%, 580px)`,
              ...
              <div ref={gridRef} className="absolute inset-0 bg-white rounded-lg border border-gray-200 overflow-hidden" onDragOver={editMode ? handleGridDragOver : undefined} onDragLeave={editMode ? handleGridDragLeave : undefined} onDrop={editMode ? handleGridDrop : undefined}>
                ...
                opacity: 0.75,
                    ...
                    {ghost.stackLabel && (<span className="text-[10px] font-bold text-green-700 bg-green-100/80 px-1 rounded leading-none select-none">
                        {ghost.stackLabel}
                      </span>)}
                  </div>)}
            ...
            return (<div key={box.id} className="absolute p-[2px]" style={boxCanvasStyle(box, xCm, yCm)}>
                      <Card className={[
                ...
                }} draggable={editMode} onDragStart={editMode
                    ? (e) => handleDragStart(e, box.id, true)
                    : undefined} onDragEnd={editMode ? handleDragEnd : undefined} onClick={() => setFocusedBoxId(box.id)}>
                        {editMode && (<div className="absolute top-0.5 left-0.5 text-gray-500/50 pointer-events-none">
                            <GripVertical className="w-3 h-3"/>
                          </div>)}
                      </Card>
                    </div>);
                ...
                {visibleBoxes.length === 0 && !ghost && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            ...
            {editMode ? (<div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              ...
              </div>) : (<div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div className="lg:sticky lg:top-4 h-fit flex flex-col gap-4 w-56">
          ...
          {focusedBox ? (<Card className="p-4 border-l-4 transition-all duration-200" style={{ borderLeftColor: focusedBox.color }}>

              <div className="flex items-start gap-2 mb-3">
                <div className="w-8 h-8 rounded-md flex-shrink-0 flex items-center justify-center border border-black/10 mt-0.5" style={{ backgroundColor: focusedBox.color }}>
                ...
                <button className="text-gray-400 hover:text-gray-600 p-0.5 rounded transition-colors flex-shrink-0 text-xs leading-none" onClick={() => setFocusedBoxId(null)} aria-label="Закрыть">
                  ✕
                </button>
              </div>
              ...
              <div className="space-y-1.5 text-xs text-gray-500 mb-3">
                ...
                {isPlaced(focusedBox) ? (<div className="flex justify-between">
                  ...
                  </div>) : (<div className="text-gray-400 italic text-center text-[11px]">
                    не размещена
                  </div>)}
              </div>
            ...
            </Card>) : (<Card className="p-4 flex flex-col items-center justify-center gap-2 text-center min-h-[120px] border-dashed">
