...
export function QRScanner({
  ...
  const startScanner = useCallback(async () => {
    ...
    try {
      ...
      setCameraError(
        "Не удалось открыть камеру. Разрешите доступ к камере или загрузите фото.",
      );
    }
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="page-container py-8 max-w-2xl">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList
            className={`grid w-full ${showViewTab ? "grid-cols-2" : "grid-cols-1"}`}
          >
            ...
            <Card>
              ...
              <CardContent>
                {scanMode === "camera" ? (
                  <div
                    id={READER_ID}
                    className="w-full min-h-[clamp(200px,45dvh,320px)] rounded-lg overflow-hidden mb-4 bg-gray-900 [&_video]:rounded-lg"
                  />
                ) : (
                  <div className="mb-4">
                    {photoPreviewUrl && (
                      <div className="rounded-lg overflow-hidden border bg-muted mb-3">
                        <img
                          src={photoPreviewUrl}
                ...
                {scanError && (
                  <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    {scanMode === "photo" && selectedFile && (
                      <Button
                        variant="outline"
                        size="sm"
                        className="mt-2"
                        onClick={() => void scanSelectedFile(selectedFile)}
                      >
                  ...
                  {qrCodeData && qrImageUrl ? (
                    <>
                      <div className="bg-white p-8 rounded-lg border-2 border-gray-200 mb-4">
                        <img
                          src={qrImageUrl}
                          alt={`QR-код коробки ${qrCodeData.boxName}`}
