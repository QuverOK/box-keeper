...
export function computeZoomMax(baseScale: number, viewportW = 800): number {
    if (baseScale <= 0)
        return ZOOM_MAX_FLOOR;
    const targetPx = computeTargetFeaturePx(viewportW);
    const forMinFeature = targetPx / (MIN_FEATURE_CM * baseScale);
    const forRefBox = computeZoomForBox(baseScale, MAX_REFERENCE_BOX_CM, MAX_REFERENCE_BOX_CM, Infinity, viewportW);
    return Math.min(ZOOM_MAX_CEILING, Math.max(ZOOM_MAX_FLOOR, forMinFeature, forRefBox));
}
export function zoomToSliderValue(zoom: number, zoomMin: number, zoomMax: number): number {
    if (zoomMin >= zoomMax)
...
export function sliderValueToZoom(value: number, zoomMin: number, zoomMax: number): number {
    if (zoomMin >= zoomMax)
        return zoomMin;
    const t = Math.min(ZOOM_SLIDER_MAX, Math.max(ZOOM_SLIDER_MIN, value)) /
        ZOOM_SLIDER_MAX;
    const logMin = Math.log(zoomMin);
    const logMax = Math.log(zoomMax);
    return Math.exp(logMin + t * (logMax - logMin));
}
export function computeZoomForBox(baseScale: number, wCm: number, dCm: number, zoomMax: number, viewportW = 800): number {
    if (baseScale <= 0 || wCm <= 0 || dCm <= 0)
        return 1;
    const targetPx = computeTargetFeaturePx(viewportW);
...
const clampZoom = (z: number, zoomMax: number) => Math.min(zoomMax, Math.max(ZOOM_MIN, z));
...
function computeLayout(roomWcm: number, roomHcm: number, viewportW: number, viewportH: number, zoom: number) {
    const baseScale = viewportW > 0 && viewportH > 0 && roomWcm > 0 && roomHcm > 0
...
export function computeScrollToBoxCenter(params: {
    ...
    const { xCm, yCm, wCm, dCm, roomWcm, roomHcm, viewportW, viewportH, zoom } = params;
    const { effScale, canvasW, canvasH, contentW, contentH } = computeLayout(roomWcm, roomHcm, viewportW, viewportH, zoom);
...
export function useCanvasViewport({ roomWcm, roomHcm, canvasRef, }: UseCanvasViewportOptions): CanvasViewport {
    ...
    const baseScale = viewportSize.w > 0 && viewportSize.h > 0 && roomWcm > 0 && roomHcm > 0
    ...
    const contentW = canvasW > viewportSize.w ? canvasW + canvasPad * 2 : viewportSize.w;
    const contentH = canvasH > viewportSize.h ? canvasH + canvasPad * 2 : viewportSize.h;
    ...
    const zoomIn = useCallback(() => setZoomState((z) => clampZoom(z * ZOOM_STEP, zoomMaxRef.current)), []);
    const zoomOut = useCallback(() => setZoomState((z) => clampZoom(z / ZOOM_STEP, zoomMaxRef.current)), []);
    const resetZoom = useCallback((target = 1) => setZoomState(clampZoom(target, zoomMaxRef.current)), []);
    const zoomToBox = useCallback((xCm: number, yCm: number, wCm: number, dCm: number) => {
        if (baseScale <= 0 || viewportSize.w <= 0 || viewportSize.h <= 0)
            return;
        const targetZoom = computeZoomForBox(baseScale, wCm, dCm, zoomMaxRef.current, viewportSize.w);
    ...
    const setViewportNode = useCallback((node: HTMLDivElement | null) => {
        ...
        const applyPreview = () => {
            ...
            const pointerX = focalX + node.scrollLeft;
            const pointerY = focalY + node.scrollTop;
            const { canvasW, canvasH, contentW, contentH } = computeLayout(roomWcm, roomHcm, vw, vh, liveZoom);
        ...
        const handleTouchMove = (e: TouchEvent) => {
            ...
            if (pinchActive && pinchPrevDist > 0) {
                ...
                liveZoom = clampZoom(liveZoom * (distance / pinchPrevDist), zoomMaxRef.current);
                if (!rafId)
