...
function centerPartitionXY(roomSize: {
    width: number;
    depth: number;
...
function centerLabelXY(roomSize: {
    width: number;
    depth: number;
}) {
    const roomWcm = roomSize.width * 100;
...
export function LayoutEditControls({ partitions, layoutLabels, roomSize, onCreatePartition, onDeletePartition, onCreateLabel, onDeleteLabel, }: LayoutEditControlsProps) {
    ...
    const resetPartitionForm = () => {
        const center = centerPartitionXY(roomSize, DEFAULT_PART_WIDTH, DEFAULT_PART_DEPTH);
    ...
    return (<div className="flex flex-wrap gap-2 items-center">
      <Dialog open={partOpen} onOpenChange={(open) => {
            setPartOpen(open);
            if (open)
                resetPartitionForm();
            if (!open)
        ...
        <DialogContent>
          ...
          {partError && (<Alert variant="destructive">
      ...
      <Dialog open={labelOpen} onOpenChange={(open) => {
            setLabelOpen(open);
            if (open)
                resetLabelForm();
            if (!open)
        ...
        <DialogContent>
          ...
          {labelError && (<Alert variant="destructive">
      ...
      {partitions.length > 0 && (<span className="text-xs text-muted-foreground">
          {partitions.length} перегородок
        </span>)}
      {layoutLabels.length > 0 && (<span className="text-xs text-muted-foreground">
          {layoutLabels.length} подписей
        </span>)}

      {(partitions.length > 0 || layoutLabels.length > 0) && (<div className="w-full flex flex-wrap gap-1 mt-1">
          {partitions.map((p) => (<div key={p.id} className="inline-flex items-center gap-1 h-7 px-2 rounded-md bg-muted text-xs">
              <span className="truncate max-w-[140px]">
                {p.label ??
                    `Перег. ${Math.round(p.width)}×${Math.round(p.depth)}`}
              </span>
              <Button type="button" variant="ghost" size="icon" className="h-5 w-5 shrink-0" aria-label="Удалить перегородку" onClick={() => onDeletePartition(p.id)}>
                <Trash2 className="w-3 h-3"/>
              </Button>
            </div>))}
          {layoutLabels.map((l) => (<div key={l.id} className="inline-flex items-center gap-1 h-7 px-2 rounded-md bg-muted text-xs">
              <span className="truncate max-w-[140px]">
                {l.text.slice(0, 20)}
              </span>
              <Button type="button" variant="ghost" size="icon" className="h-5 w-5 shrink-0" aria-label="Удалить подпись" onClick={() => onDeleteLabel(l.id)}>
                <Trash2 className="w-3 h-3"/>
              </Button>
            </div>))}
        </div>)}
