...
export function Storage3DView({
  ...
  useEffect(() => {
    if (!refitOnResizeRef.current) return;
    ...
    <div className="flex items-center gap-1.5">
      ...
      <Slider
        ...
        onValueChange={(values: number[]) =>
          setZoom(sliderValueToZoom(values[0], ZOOM_MIN, zoomMax))
        }
        aria-label="Масштаб"
      ...
      <Button
        ...
        size="icon"
        className="w-8 h-8"
        onClick={() => resetZoom(resetZoomTarget)}
  ...
  const stagingList =
    unplacedBoxes.length === 0 ? (
      ...
      <div
        className={cn(
          "flex gap-3 p-3 overflow-x-auto pb-1 min-h-[96px] rounded-lg transition-colors",
      ...
      >
        {unplacedBoxes.map((box) => {
          ...
          return (
            ...
            >
              <Card
                className={cn(
                  "w-28 h-24 transition-all relative overflow-hidden select-none border shadow-sm",
                  editMode
                    ? isDragging
                      ? "cursor-grabbing"
                      : "cursor-grab"
                ...
                onMouseEnter={
                  editMode ? () => setHoveredBoxId(box.id) : undefined
                }
                onMouseLeave={
                  editMode ? () => setHoveredBoxId(null) : undefined
                }
              ...
              >
                ...
                <div
                  className="p-2 h-full flex flex-col items-center justify-center text-center"
        ...
        {editMode && (
          <div className="flex-shrink-0 w-28 h-24 border-2 border-dashed border-border rounded-lg flex items-center justify-center">
  ...
  const boxInfoCard = focusedBox && (
    <Card className="p-4 border transition-all duration-200">
      <div className="flex items-start gap-2 mb-3">
        <div
          className="w-8 h-8 rounded-md flex-shrink-0 flex items-center justify-center shadow-sm mt-0.5"
        ...
        <button
          className="text-muted-foreground hover:text-foreground p-0.5 rounded transition-colors flex-shrink-0 text-xs leading-none"
  ...
  const canvasRegion = (
    ...
    >
      <div
        ref={setViewportNode}
        className={cn(
          "miro-scroll relative h-full w-full overflow-auto bg-muted [touch-action:pan-x_pan-y]",
      ...
      >
        ...
        >
          <div
            ref={gridRef}
            className={cn(
              "absolute bg-card rounded-lg border border-border overflow-visible",
            ...
            {sortedPlacedBoxes.map((box) => {
              ...
              return (
                <div
                  ...
                  style={wrapperStyle}
                  onClick={
                    isInExpandedStack ? (e) => e.stopPropagation() : undefined
                  }
                >
                  <DraggableBox
                    id={box.id}
                    ...
                    className={getBoxHighlight(box.id, {
                      isDragging,
                      isStackExpanded: isInExpandedStack,
                    })}
                    draggable={editMode && !spaceHeld}
                    onDragStart={
                      editMode
        ...
        <div className="relative h-full w-full overflow-hidden">
          ...
          <div className="absolute z-20 left-3 top-3 sm:top-auto sm:bottom-3 max-w-[calc(100%-1.5rem)]">
            <button
              ...
              className="flex items-center gap-2 bg-card/95 backdrop-blur border border-border shadow-lg rounded-full px-3 py-1.5 text-sm font-medium"
            >
            ...
            {stagingOpen && (
              <div
                className={cn(
                  "mt-2 bg-card/95 backdrop-blur border border-border shadow-lg rounded-xl p-3 max-w-[80vw] transition-colors",
                  editMode &&
                    isStagingDragOver &&
