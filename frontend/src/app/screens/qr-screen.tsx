...
export function QrScreen() {
    ...
    const boxId = "boxId" in params ? (params.boxId as string | undefined) : undefined;
    ...
    const qrCodeData = box && boxId
        ? {
            boxName: box.name,
            storageName: (box as typeof box & {
                storage?: {
                    name: string;
                };
            }).storage
        ...
        : undefined;
    const handleScanSuccess = useCallback(async ({ storageId, boxId: scannedBoxId, }: {
        storageId: string;
        boxId: string;
    }) => {
    ...
    return (<>
      <QRScanner key={scannerKey} onBack={() => navigate({ to: isAuthenticated ? "/dashboard" : "/login" })} onScanSuccess={handleScanSuccess} qrCodeData={qrCodeData} showViewTab={isAuthenticated && !!boxId}/>

      <AlertDialog open={pendingForeignScan !== null} onOpenChange={(open) => {
            if (!open) {
                setPendingForeignScan(null);
                setScannerKey((k) => k + 1);
            }
