...
import { useStorage, usePublicStorage, useUpdateStorage, } from "@/entities/storage";
import { useCreateBox, useMoveAnyBox } from "@/entities/box";
import { api } from "@/shared/api/client";
import { BOXES_KEY, STORAGES_KEY } from "@/shared/api/query-keys";
import { useCreatePartition, useDeletePartition, useUpdatePartition, } from "@/entities/partition";
import { useCreateLayoutLabel, useDeleteLayoutLabel, useUpdateLayoutLabel, } from "@/entities/layout-label";
...
export function StorageScreen() {
    ...
    const requireAuth = () => {
        const redirect = stripGuestFromPath(window.location.pathname + window.location.search);
    ...
    return (<StorageView storageName={storage.name} boxes={(storage.boxes ?? []).map((b) => ({
            id: b.id,
            name: b.name,
            x: b.posX ?? undefined,
            y: b.posY ?? undefined,
        ...
        }} readOnly={readOnly} highlightBoxId={highlightBoxId} onRequireAuth={requireAuth} onBack={() => navigate({ to: readOnly ? "/login" : "/dashboard" })} onBoxClick={(boxId) => navigate({
        ...
        }} partitions={storage.partitions ?? []} layoutLabels={storage.layoutLabels ?? []} onCreatePartition={async (data) => {
