...
export function BoxScreen() {
    ...
    const roomSize = readOnly && publicStorage
        ? {
        ...
        : authStorage
            ...
            : undefined;
    const siblingBoxes = (readOnly ? publicStorage?.boxes : authStorage?.boxes)?.map((b) => ({ id: b.id, name: b.name }));
    const availableCategories = extractStorageCategories(readOnly ? (publicStorage?.boxes ?? []) : (authStorage?.boxes ?? []));
    const requireAuth = () => {
    ...
    return (<BoxView boxId={boxId} boxName={box.name} boxColor={box.color} boxSizeW={box.sizeW} boxSizeD={box.sizeD} boxSizeH={box.sizeH} qrCode={box.qrCode} roomSize={roomSize} siblingBoxes={siblingBoxes} readOnly={readOnly} onRequireAuth={requireAuth} availableCategories={availableCategories} items={(box.items ?? []).map((i) => ({
            id: i.id,
            name: i.name,
            category: i.category,
