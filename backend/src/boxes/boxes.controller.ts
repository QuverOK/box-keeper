...
export class BoxesController {
  ...
  findAll(@Param("storageId") storageId: string, @Request() req) {
  ...
  @Patch("boxes/:id")
  update(@Param("id") id: string, @Request() req, @Body() dto: UpdateBoxDto) {
    return this.boxesService.update(id, req.user.id, dto);
  }

  @Patch("boxes/:id/position")
  move(@Param("id") id: string, @Request() req, @Body() dto: MoveBoxDto) {
