...
export class BoxesService {
    constructor(private readonly prisma: PrismaService, private readonly storagesService: StoragesService) { }
    private async assertUniqueName(storageId: string, name: string, excludeId?: string) {
        ...
        if (existing) {
            throw new ConflictException("Коробка с таким названием уже существует в этом хранилище");
        }
    }
    private assertBoxFitsRoom(sizeW: number, sizeD: number, sizeH: number, storage: {
        roomWidth: number;
        roomDepth: number;
        roomHeight: number;
    }) {
        ...
        if (sizeH > roomHcm)
            errors.push("высота");
        if (errors.length > 0) {
            throw new ConflictException(`Размер коробки превышает размеры помещения (${errors.join(", ")})`);
        }
    }
